...
class Water():
  def __init__(self, pos, number_of_molecules, separation, height) -> None:
    self.pos = pos
    self.molecules = []
    self.spread = 0.2
  ...
  def update(self, scroll, player_rect):
    ...
    self.points.append(POINT(self.pos[0] + self.radius * (self.number_of_molecules-1) - 16 - scroll[0], self.pos[1] + self.height - scroll[1]))
